...
def remove_spaces(strin):
    ...
    r_found = False
    r = len(strin)
    while not l_found and l < r:
        if strin[l] == " ":
            l += 1
        else:
            l_found = True

    while not r_found and r > l:
        if strin[r-1] == " ":
            r -= 1
        else:
            r_found = True
...
def normalise_input(user_input):
    ...
    for n in user_input:
        if n in list(string.punctuation):
            continue
        else:
            new_user += n
...
def menu(exits):
    ...
    while True:
        ...
        # Read player's input
        print()
        dir_input = str(input("Please enter the direction of travel: "))

        # Normalise the input
        #dir_input = normalise_input(dir_input)

        # Check if the input makes sense (is valid exit)
        try:
            if dir_input.lower() in dirs:
                print("You are now in " + exits[dir_input])
                break
...
def move(exits, direction):
    ...
    False
    """
    exits_one = exits[direction.lower()]
    return rooms[exits_one]
